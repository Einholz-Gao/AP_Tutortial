...
public:
  ...
  Matrix(std::size_t rows, std::size_t cols, T uniform_value)
    ...
    // std::generate(begin(), end(), [](){return uniform_value;});
    std::fill(begin(),end(), uniform_value);
  }

  Matrix() = delete;

  T &operator()(int i, int j) { return _arr[i * _cols + j]; }

  //Why here a const operator overload?
  const T &operator()(int i, int j) const { return _arr[i * _cols + j]; }
  ...
  Matrix(const Matrix &from) : _rows(from._rows), _cols(from._cols) {
  ...
  Matrix &operator=(const Matrix &from) {
    ...
    if (_rows == from._rows && _cols == from._cols) {
      ...
      throw std::invalid_argument(
          "The two matrices need to have the same size to be assigned");
    }
