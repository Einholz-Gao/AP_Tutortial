...
std::vector<bool> get_neighbors(Matrix<bool> &domain, int i, int j) {
  ...
  // + o +
  // + + +
  return std::vector<bool>{domain(i - 1, j),     domain(i + 1, j),
                           domain(i, j - 1),     domain(i, j + 1),
                           domain(i - 1, j - 1), domain(i + 1, j + 1),
                           domain(i - 1, j + 1), domain(i + 1, j - 1)};
}
  ...
  for (auto t = 0; t < epochs; t++) {
    ...
    for (auto i = 1; i < N - 1; i++) {
      for (auto j = 1; j < N - 1; j++) {
        ...
        sums(i-1, j-1) = std::accumulate(neighbors.begin(),neighbors.end(), 0);
        // evolve(sums(i-1,j-1));
      }
    }
